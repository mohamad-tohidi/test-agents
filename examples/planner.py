...
def main():
    ...
    agent = CodeAgent(
        ...
        tools=[
            DuckDuckGoSearchTool()
        ],  # Add a search tool for more interesting plans
    ...
    try:
        ...
        if "interrupted" in str(e).lower():
            ...
            print(
                "agent.run(task, reset=False)  # This preserves the agent's memory"
            )
            ...
            resume_choice = (
                input("\nWould you like to see resume demonstration? (y/n): ")
                .strip()
                .lower()
            )
            if resume_choice == "y":
